...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        A, self.cache['hidden'] = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        scores, self.cache['scores'] = affine_forward(A, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dA, grads['W2'], grads['b2'] = affine_backward(dscores, self.cache['scores'])
        dx, grads['W1'], grads['b1'] = affine_relu_backward(dA, self.cache['hidden'])
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        A, self.cache['layer1'] = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        if(self.num_layers > 1):
            curr_layer = 2
            for i in range(self.num_layers - 2):
                A, self.cache['layer' + str(curr_layer)] = affine_relu_forward(A, self.params['W' + str(curr_layer)], self.params['b' + str(curr_layer)])
                curr_layer += 1
            A, self.cache['layer' + str(curr_layer)] = affine_forward(A, self.params['W' + str(curr_layer)], self.params['b' + str(curr_layer)])
